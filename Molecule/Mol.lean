...
/--
Theorem: The cusp (1/4) is in the main molecule.
We prove this by showing it is the image of z=1/2, which lies in the closure of the parameterizing disk.
-/
theorem cusp_in_Mol : cuspVal ∈ MolSet := by
  rw [MolSet, mainCardioid_eq_image]
  have h_val : mainCardioidMap 0.5 = cuspVal := by
    simp [mainCardioidMap, cuspVal]
    norm_num
  rw [← h_val]
  apply image_closure_subset_closure_image (continuous_id.sub (continuous_id.pow 2))
  rw [closure_ball (0:ℂ) (by norm_num)]
  use 0.5
  constructor
